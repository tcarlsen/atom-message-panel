    ...
    this.div({class: 'am-panel tool-panel panel-bottom'}, function () {
        this.div({class: 'panel-heading'}, function () {
            ...
            this.div({class: 'heading-buttoms inline-block pull-right'}, function () {
                ...
                this.div({class: 'heading-close inline-block icon-x', style: 'cursor: pointer;', outlet: 'btnClose', click: 'close'});
            }.bind(this));
        }.bind(this));
...
MessagePanelView.prototype.fold = function () {
    this.btnFold.removeClass('icon-fold');
    this.btnFold.addClass('icon-unfold');
    this.body.hide(this.speed);
    this.summary.css('display', 'inline-block');
    this.folded = true;
};

MessagePanelView.prototype.unfold = function () {
    this.btnFold.removeClass('icon-unfold');
